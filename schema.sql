...
CREATE POLICY "Lojistas can create reviews for their own chamados."
ON avaliacoes
FOR INSERT
WITH CHECK (
  (SELECT role FROM profiles WHERE profiles.id = auth.uid()) = 'lojista' AND
  lojista_id = auth.uid() AND
  EXISTS (SELECT 1 FROM chamados WHERE chamados.id = chamado_id AND chamados.lojista_id = auth.uid())
);
